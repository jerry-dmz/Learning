...
| ----- | ------ | ------------------------------------------------------------ |
| 0     | FF0h   | 磁盘标识                                                     |
| 1     | FFFh   | 第一个簇已经被占用。为何 TODO:                               |
| ...   | ...    | 取不同值，有不同的意思。<br /><br />**000h**:可用簇<br /><br />**002h-FEFh**:已用簇，标识下一个簇的簇号<br /><br />**FF0h-FF6h**:保留簇<br /><br />**FF7h**:坏簇<br /><br />**FF8-FFFh**：文件的最后一个簇 |
...
**书虽然还不错，但是很多地方说的很模糊，很多地方感觉作者就是一笔带过，自己可能都没深究。**

# 《Orange's:一个操作系统的实现》

**讲的比较详细，讲究细节，而且比较好懂，基本和上本书是一脉相承的，不知道是谁借鉴谁的了。**
...
与GDT类似，使用lldt加载，选择子中ti决定了使用全局还是局部表
...
* G位：**Granularity**,段界限粒度位，为0时代表粒度为字节，否则为4KB
* D/B位：不同的段代表不同意思。
  * 可执行代码描述符，叫做D位。为1时，在默认情况下指令使用32位地址及32位或8位操作数；否则使用16位地址及16位或8位操作数
  * 对于向下扩展的数据段，叫做B位。为1时，段的上部界限为4GB,否则为64KB
  * 堆栈段，叫做b位。为1时，隐式的堆栈访问指令（如push、pop或call等），使用esp;否则使用sp
* AVL位：保留位
...
总容易对这个东西提不起兴趣。到底被卡在那里了呢？是担心学了没有回报吗？还是担心自己学不会？为什么要学这个东西?为什么总是一拖再拖？为什么总是达不到想要的结果？为什么总是容易偷懒打滑？

心若冰清，天塌不惊。定了目标，就该一往无前的去努力，去实现，至于前路是否宽广，是否有意义，要试过之后才知道。我一定会成功攻克这个难关，然后走向其他的一个个目标。若感人生无味，那不妨自己加加味，更强、更透彻。人生有很多意义，只要能始终坚持其中一条，就很不容易了。

不要沉迷于照着代码敲一遍了，已经过了这个阶段，要有一些更高效的学习方式。要理解每一句话的意思，以及这句话说的对不对，要提出更多的问题，不怕问题多，就怕没问题，但是什么也不会。学完之后，则是应该学以致用，要脱离之前学习的边界，要有自己独到的见解。
...
**xp /33bx 0x7c00**  查看0x7c00~0x7c21的的内容，内容以16进制展示（**x**），每次展示1字节（**b**）

**sreg**，查看cs、cs、ss等寄存器的值。reg查看普通寄存器，creg查看控制寄存器。

**xchg bx,bx**，在代码中加上此此段代码，和breax xxx的都一样，但是不会被info break显示（暂时还不清楚原理）**Magic point**

**u start end**，反汇编一段物理内存
...
mov ax,cs
bochs中调试：
[0x32342] 3224:0102 (unk. ctxt):jmp .+667 (0x325e0) ;e99b02
n
[0x325e0] 3224:03a0 (unk. ctxt):mov ax,cs           ;8cc8

ox325e0-32442=670,与667并不相符，但是跳转指令本身就占3字节，所以这个667的偏移距离是减去了指令长度的。
```

### 寄存器

x86系列从8086到现在，有哪些扩充，assembly.md中比较详细

### 特权级

#### CPL

当前执行程序/任务的特权级。存储在**cs和ss的第0位和第1位**。程序转移到不同特权级的代码段时，处理器将改变**CPL**。

但是**一致性代码段**，可以被相同或低特权级代码访问，此时**CPL不会改变**

#### DPL

段或门的特权级，存储在**段描述符或门描述符的DPL字段**。

* 数据段，规定了能访问此段的**最低特权级**。如：1，只有0或1的段才能访问
* 非一致性代码段，规定访问此段的**特权级**。
* 一致性代码段，规定访问此段的**最高特权级**。如：2，那么0或1无法访问此段。
* 调用门，规定了访问此段的**最低特权级**。
* 通过调用门访问的非一致性代码段，规定访问此段的**最高特权级**.
* tss，**最低特权级**。

#### RPL

段选择子的第0位和第1位表现，通过检查**RPL**和**CPL**来确认访问是否合法，即便提出访问请求的段有足够的特权级。

场景：

操作系统过程（**0**）从一个应用程序(**1**)接受选择子时，会把RPL设置为调用者的特权级(**1**)。操作系统用这个选择子去访问对应的段时，就会将调用过程的特权级，而非操作系统的特权级

程序从一个段转移到另一个段之前，目标代码段的选择子会被加载到cs中，**加载过程中会检查描述符**的界限、类型、特权级等内容。

程序控制转移的发生，可以由jmp、call、ret、sysenter、sysexit、int n、iret等指令引起，也可以由中断或异常机制引起。其中使用jmp、call可以实现以下4种转移：
...
![image-20240714125834521](.\os.assets\门描述符.png)

一个门描述了由一个选择子和一个偏移所指定的线性地址。门的分类有4种：
...
也是**GDT**中一个表项，8个字节，也需要通过选择子机制访问，只不过结构不同而已。

**call**时直接使用**门描述符中定义的选择子**也能达到一样的效果。

通过调用门和call指令，可以实现低特权级到高特权级的转移，无论目标代码段是一致的还是非一致的。

为什么这么设计，其实没有看太懂，TODO:???

### 堆栈

对于，jmp，长跳转（段间）、段跳转（段内）没什么区别。而call指令会影响堆栈，长、短跳转对堆栈的影响是不同的。

比如：对于段内跳转，call指令会将下一条指令eip入栈，，ret指令执行时，会将eip弹出。

每个任务最多可能在4个特权级之间转移，每个任务可能需要4个堆栈

#### TSS

**Task State Stack**

![image-20240720163716795](./os.assets/image-20240720163716795.png)

#### 转移过程

##### call

1. 根据目标代码段的DPL,从tss中选择应该切换到那个ss、esp。
2. 从tss中读取新的ss和esp，这过程发现ss、esp或者tss界限错误都会导致无效TSS异常。
3. 对ss进行合法性校验
4. 暂时性保存ss、esp的值
5. 加载新的ss、esp
6. 将第3步临时保存的ss、esp的值压入新栈
7. 从调用者堆栈将参数复制到被调用堆栈中，复制参数的数目由调用门中Params Count一项决定
8. 将当前cs、eip压栈
9. 加载调用门中指定的cs、eip

##### ret

1. 检查cs的RPL以判断返回时是否要变换特权级
2. 加载被调用者堆栈上的cs、eip（此时会对cs进行校验）
3. 如果ret指令含有参数，则增加esp的值以跳过参数，然后esp将指向被保存过的调用者ss和esp。ret的参数必须对应调用门中**Params Count**
4. 加载ss和esp，切换调用者堆栈，被调用者的ss、esp被丢弃
5. 如果ret指令有参数，增加esp的值以跳过参数

##### 实例分析2

```assembly
;retf指令的本质，下面这些动作本来是call之前就做好了的，但是也可以手动做。
push stackSelector	; 往堆栈中push要返回的ss
push 0x1ff			; 往堆栈中push要返回的esp
push codeSelector	; 往堆栈中push要返回的cs
push 0				; 往堆栈中push要返回的eip
retf				; 返回
```
...
中断通常在程序执行时因为硬件而随机发生，通常用来处理**处理器外部**的事件，比如外围设备的请求，软件通过init n指令也可产生中断。

异常则通常在处理器执行指令过程中检测到错误时发生，比如页错误。

20~31号中断号被保留，32~255为用户自定义中断。

* **Fault**,一种可被更正的异常，被更正后程序能不失连贯性的继续执行。当Fault发生时，处理器会把产生fault的指令之前的状态保存起来，异常处理程序返回地址为fault的指令。
* **Trap**,一种在trap指令执行之后立即被报告的异常，也允许程序或任务不失连贯性的继续执行。异常处理程序的返回地址变成trap指令之后那条指令。
* **Abort**,一种不总是报告精确异常发生位置的异常，不允许程序或任务继续执行，而是用来报告严重错误的。

#### 中断描述符表

**IDT**(Interrupt Descriptor Table)包含下面三种表项

* 中断门描述符
* 陷阱门描述符
* 任务门描述符

IDT的作用是将每一个中断向量和一个描述符对应起来。调用门通过call指令调用，中断门和陷阱则是通过**int**指令。

<img src="./os.assets/image-20240908144604246.png" alt="image-20240908144604246" style="zoom:50%;" />
...
也叫外部中断，需要建立硬件中断和向量号之间的对应关系。分为不可屏蔽中断，可屏蔽中断，分别有CPU的NMI（**NMI**）、INTR引脚来接受。

![image-20240908151249621](./os.assets/image-20240908151249621.png)

##### NMI

Non-Maskable Interruput

不可屏蔽，它与IF是否被设置无关，对应中断向量号为2.

##### INTR

可屏蔽中断与CPU的关系是通过对**8259A**建立起来的。

###### 8259A

TODO:

* 中断管理，管理多个中断源，接受来自外部设备的中断请求，并将这些请求进行优先级排队
* 中断屏蔽，可以通过设置中断屏蔽字来屏蔽某些特定的中断请求
* 中断嵌套，当一个高优先级正在被处理时，如果有更高优先级的中断请求，处理器可以暂停当前处理的中断处理程序，转而处理更高优先级的中断。

BIOS初始化时，IRQ0~IR7被设置为对应向量号08h~0Fh,因此要重新设置

## FAT12

早期在软盘上使用的文件系统。

<img src="./os.assets/image-20240916161212434.png" alt="image-20240916161212434" style="zoom:50%;" />

* 扇区，磁盘上最小数据单元
* 簇，一个或多个扇区
* 分区

引导扇区定义了磁盘的元数据，比如一簇多少扇区，没扇区字节数，根目录最多多少个条目

### 引导扇区

不仅包含了引导程序，还有FAT12文件系统的整个**组成结构信息**。这些信息描述了FAT12文件系统对磁盘扇区的管理情况，相当于EXT文件系统的**superblock**结构。<img src="./os.assets/image-20240916125852448.png" alt="image-20240916125852448" style="zoom:50%;" />

### FAT表

<img src="./os.assets/image-20240916162724611.png" alt="image-20240916162724611" style="zoom:50%;" />

表项宽度与FAT类型有关，第0、1项不作为数据区大的索引值使用。

### 根目录区/数据区

根目录区只能保存目录项信息，数据区不但可以保存目录项，还能保存文件内的数据。

<img src="./os.assets/image-20240916163225714.png" alt="image-20240916163225714" style="zoom:50%;" />

### 实验2 探究Fat12文件系统

借助bochs创建一个虚拟软盘，使用freedos格式为fat12文件系统的软盘，然后后xdd查看文件内容（即扇区内容）。

* bximage创建一个虚拟软盘
* 修改freedos的配置文件，将刚刚虚拟软盘设置为c盘
* 执行format命令

对于1.44MB的软盘，总共有两面（磁头号0和1），每面80个磁道，每个磁道18个扇区。

**1.44MB=2 x 80 x 18 x 512 = 1.44MB**

![image-20240916175942110](./os.assets/image-20240916175942110.png)

#### 实验2.1 观测格式化后的虚拟磁盘文件

![image-20241003194607133](./os.assets/image-20241003194607133.png)

#### 实验2.2 观测格式化后的虚拟磁盘文件-小文件

![image-20241003204137717](./os.assets/image-20241003204137717.png)

#### 实验2.3 观测格式化后的虚拟磁盘文件-小文件

#### 实验2.4 观测格式化后的虚拟磁盘文件-大文件

### 实验3 探究实模式下中断

探究实模式下的中断及其实现机制。TODO:
...
通过int 0x80h调用对应的系统调用

```assembly
nasm -f elf test1.asm -o hello.o;
ld -s -m elf_i386 hello.o -o hello;其中-s选项会去除符号表
```

为什么这么长时间，对于这个操作系统的编写一直都是断断续续的，这其中到底有什么困难，我到底被什么难住了呢？而且现在遇到的这个就是有一股抵触情绪，很难有让人有成就感，很难形成一个正循环。

不要拖延，要坚持，要相信一定会有云开见日出的一天，达成目的有很多途径，不要在一颗树上吊死，要从多角度思考问题，我一定会克服重重难关，到达彼岸。

君子慎独，不欺暗室。

### 实验5 c调用汇编程序

参照scratchLoader,要注意其中c函数被翻译成的汇编代码，在不同gcc不同版本可能是不同。
